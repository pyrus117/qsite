...
- **`www.qyouthnz.com` works again as of 2026-09-01** — AAAA `www`→`100::` (proxied) plus a redirect rule, both added in the dashboard. Verified live: `https://www.qyouthnz.com/drop-ins` 301s to `https://qyouthnz.com/drop-ins`, so the path survives the hop. It had no DNS record at all from the Cloudflare migration until this fix, so ignore any older note calling `www.` links dead. Absolute metadata URLs still use the apex — the redirect serves visitors, not canonicals
- **SEO (2026-09-01 pass):** every page also carries `og:site_name`, `og:image:alt`, `twitter:image`, `twitter:image:alt` and `theme-color`; `BreadcrumbList` JSON-LD sits on all 9 inner pages, `FAQPage` on drop-ins, `Service` on education, `NGO`+`WebSite` on the homepage. Post pages get `BlogPosting` + `BreadcrumbList` from `worker/blogPages.ts`, which **strips every JSON-LD block out of the blog.html template first** — so schema added to blog.html never reaches a post page, by design. `public/_headers` caches `/images/*` and `/resources/*` for a day (Workers Assets otherwise serves everything `max-age=0, must-revalidate`); `styles.css`, `site-content.js` and `site-data.json` stay revalidating on purpose because they are unversioned
