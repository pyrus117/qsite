...
- Canonical/og/schema/sitemap URLs use the **apex domain** (`https://qyouthnz.com`) and are **extensionless** (`/blog`, not `/blog.html`) — the live server 301s www→apex and Cloudflare's asset layer 307s `*.html`→extensionless, so never put `www.` or `.html` in absolute metadata URLs (relative nav links keep `.html`; the redirect handles them)
